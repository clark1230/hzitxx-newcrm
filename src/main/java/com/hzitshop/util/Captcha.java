...
public class Captcha {
	...
	public static Captcha init(HttpServletResponse response, HttpServletRequest request) {
		return new Captcha(response,request);
	}
	
	private Captcha(HttpServletResponse response, HttpServletRequest request){
	...
	public static void setCaptchaName(String captchaName) {
		if (StringUtil.isNotEmpty(captchaName)) {
	...
	public void render() {
		...
		request.getSession().setAttribute("validateCode",vCode);//包验证码保存到session中
	...
	public static boolean validate(HttpServletRequest request, HttpServletResponse response, String userInputCaptcha) {
		if (StringUtil.isNotEmpty(userInputCaptcha)) {
			return false;
		}
